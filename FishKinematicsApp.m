...
%       Statistics and Machine Learning Toolbox  (nanmean/nanstd replaced)
    ...
    %% ---- Figure ----
    % Startup size: centered on screen, NOT maximized, targeting a 9:16
    % (portrait) aspect ratio. A strict 9:16 window sized to fit a typical
    % laptop screen height would come out narrower than this app's
    % minimum usable width (minW below — the left panel alone is 350px,
    % and several tabs have fixed-pixel content that needs real width to
    % avoid the exact cut-off problem the Batch tab just had). So this
    % targets 9:16 but won't go narrower than minW even if that means
    % departing from a perfect 9:16 ratio on a short screen — a working,
    % slightly-off-ratio window beats a precisely-proportioned broken one.
    minW = 800; minH = 560;
    screenSize = get(groot, 'ScreenSize');   % [left bottom width height]
    targetH = min(screenSize(4) * 0.85, 1200);
    targetW = targetH * 9/16;
    if targetW < minW
        targetW = minW;
        targetH = min(targetW * 16/9, screenSize(4) * 0.92);
    end
    startX = screenSize(1) + (screenSize(3) - targetW) / 2;
    startY = screenSize(2) + (screenSize(4) - targetH) / 2;

    fig = uifigure('Name',     'Kinemetrix', ...
                   'Position', [startX startY targetW targetH], ...
                   'Color',    [0.95 0.95 0.95], ...
                   'AutoResizeChildren', 'off');
    % Minimum usable size — prevents content from collapsing below this
    fig.UserData.minW = minW;
    fig.UserData.minH = minH;
